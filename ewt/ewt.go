...
type Token struct{
	SigResponse SignatureResponse
	Expirydate time.Time
	Valid bool
}
func (t *Token) FromJSON(jsonStr string) error {
	return json.Unmarshal([]byte(jsonStr), t)
}
func (t *Token) String() string {
	return fmt.Sprintf("SigResponse: {Sig: %s, Address: %s, Msg: %s}, ExpiryDate: %s, Valid: %t",
		t.SigResponse.Sig, t.SigResponse.Address, t.SigResponse.Msg, t.Expirydate, t.Valid)
}
